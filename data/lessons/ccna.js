...
CertHub.addLessons("ccna", [
 ...
 },
 {
  ...
  "body": [
   ...
   "The 5 GHz band offers many more non-overlapping 20 MHz channels, which makes it easier to avoid co-channel interference, and supports channel bonding into 40, 80 or 160 MHz channels for higher throughput. The trade-off is shorter range and weaker penetration. Some 5 GHz channels require dynamic frequency selection (DFS), meaning the AP must move off a channel if it detects radar. The 6 GHz band, opened for Wi-Fi 6E and later, adds a large block of clean spectrum with many wide channels and no legacy devices, but has even shorter range and only newer clients can use it. WPA3 is required for 6 GHz operation.",
 ...
 },
 {
  ...
  "body": [
   ...
   "SVIs are faster and scale better than router-on-a-stick because routing happens inside the switch at wire speed with no trunk bottleneck, which is why campus distribution layers use them. When troubleshooting, check the subinterface VLAN number matches the switch VLAN, the trunk allows the VLAN, the physical router interface is not shut down, `ip routing` is enabled on the switch, and hosts use the correct gateway. `show ip interface brief` and `show ip route` should list each VLAN's subnet as connected (C) with a local (L) route for the gateway address."
  ],
  "terms": [
   [
    "Router-on-a-stick",
 ...
 },
 {
  ...
  "body": [
   ...
   "Mitigations come in layers, often called defense in depth. Technical controls include patching, strong passwords and multifactor authentication (MFA, combining something you know, have and are), ACLs, firewalls, intrusion prevention systems, port security, DHCP snooping, dynamic ARP inspection, encryption and disabling unused services. Physical controls include locked wiring closets, badge readers and cameras. Administrative controls include policies, procedures and background checks.",
