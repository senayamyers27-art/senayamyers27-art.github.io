...
CertHub.addLessons("ccnp-encor", [
 ...
 },
 {
  ...
  "body": [
   ...
   "The mapping system stores which EIDs live behind which RLOCs. It has two functions, often run on the same device. The map server (MS) receives registrations: LISP routers send Map-Register messages saying 'these EID prefixes are reachable via my RLOC.' The map resolver (MR) receives Map-Request queries from routers that need to find an EID and answers them, either directly or by forwarding the request toward the authoritative router. In SD-Access, the control plane node runs both roles.",
 ...
 },
 {
  ...
  "body": [
   ...
   "Neighbor states progress: Idle (not trying, or waiting after an error), Connect (attempting the TCP connection), Active (TCP failed; retrying and listening), OpenSent (TCP up, OPEN message sent), OpenConfirm (OPEN received and parameters acceptable, waiting for KEEPALIVE), and Established (exchanging UPDATE messages). A neighbor flipping between Idle and Active usually means TCP cannot complete: a wrong neighbor address, a wrong AS number (the peer rejects the OPEN), an ACL blocking TCP 179, or missing reachability. In `show ip bgp summary`, an Established neighbor shows a number of received prefixes in the State/PfxRcd column instead of a state name.",
  ...
  "check": [
   [
    ...
    "The TCP session on port 179 cannot complete because of a wrong neighbor address or no route, an ACL blocking TCP 179, or a remote-as mismatch."
   ],
   [
