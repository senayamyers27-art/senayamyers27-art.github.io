...
CertHub.addLessons("fortinet-fortigate", [
 {
  ...
  "body": [
   ...
   "Each interface also has an Administrative Access setting that lists the management services allowed to reach the FortiGate on that interface: HTTPS and SSH for management, PING so the interface answers ICMP echo, plus options such as HTTP, SNMP, FMG-Access and FortiTelemetry (used for the Security Fabric). This is a common source of lockouts: if you remove HTTPS from the interface you are managing over, you lose the GUI. On a WAN interface facing the Internet you normally allow little or nothing, and you rely on trusted hosts and a VPN for remote management.",
 ...
 },
 {
  ...
  "body": [
   ...
   "The password policy sets complexity and lifetime rules, minimum length, required character classes, and expiry, applied to admin (and optionally user) passwords. It raises the cost of guessing but again says nothing about source or authorization.",
 ...
 },
 {
  ...
  "body": [
   ...
   "By default a FortiGate runs with a single VDOM called root, and multi-VDOM mode is off. In this state you often do not even see the word VDOM in the GUI; you just configure the box. When you enable multi-VDOM (in the CLI, `config system global`, `set vdom-mode multi-vdom`, or via the GUI), the root VDOM remains and you can add more VDOMs. The root VDOM is special: it is where the management interface and global, box-wide settings live, so you keep it for administration and management traffic.",
 ...
 },
 {
  ...
  "body": [
   ...
   "Joining a downstream FortiGate follows a clear, two-sided handshake. On the downstream unit you enable the Security Fabric connection and point it at the upstream (root) unit's IP address. The connection uses FortiTelemetry, so the upstream interface must permit that access. The join does not complete until the root authorizes the downstream device, which the administrator does on the root by approving the unit's serial number. This authorization step is what prevents an unknown device from silently joining the Fabric. Simply sharing a FortiCare account or building a VPN tunnel does not create a Fabric.",
 ...
 },
 {
  ...
  "body": [
   ...
   "Memory pressure has a specific mechanism. When free memory falls below a red threshold, the FortiGate enters conserve mode to protect itself: it stops accepting new proxy-based inspection sessions and may drop or bypass some work until memory recovers. The `av-failopen` global setting decides what happens to traffic that needs proxy-based antivirus during conserve mode: pass lets it through uninspected (availability over security), off drops it (security over availability), and one-shot passes traffic once then drops until recovery. This is distinct from IPS fail-open, which governs the IPS engine, so read the question carefully.",
  ...
  "terms": [
   ...
   [
    "av-failopen",
    "The global setting deciding whether traffic needing proxy antivirus passes uninspected (pass), is dropped (off), or passes once (one-shot) during conserve mode."
   ],
   [
    "diagnose debug flow",
 ...
 },
 {
  ...
  "terms": [
   ...
   [
    "Policy ID",
    "A stable, non-reused label assigned at creation; it identifies a policy in logs and CLI but does not set matching order."
   ],
   [
    "Sequence",
 ...
 },
 {
  ...
  "example": "A public web server is hit by a SYN flood, so the admin adds a DoS policy on the WAN interface with a tcp_syn_flood threshold, which drops the excess half-open connections before they reach firewall policy lookup.",
 ...
 },
 {
  ...
  "check": [
   [
    ...
    "get router info routing-table all, which displays the active routing table (FIB)."
   ],
   [
 ...
 },
 {
  ...
  "body": [
   ...
   "FortiGate supports two RPF modes. Strict (feasible) RPF requires that the best route back to the source uses the same interface the packet arrived on. Loose RPF is more permissive: it only requires that some route back to the source exists in the routing table, on any interface. FortiOS commonly uses a feasible-path check by default, which accepts the packet if any route toward the source exists via that interface (helpful with multiple paths), but strict environments can tighten it. The key concept for the exam is that RPF ties acceptance to the existence of a return route on the ingress interface.",
  ...
  "terms": [
   ...
   [
    "Loose/feasible RPF",
    "A more permissive mode that accepts the packet if any return route to the source exists (on any interface, or via the ingress interface)."
   ],
   [
    "Asymmetric routing",
  ...
  "check": [
   ...
   [
    ...
    "Strict requires the best return route to use the ingress interface; loose only requires that some return route to the source exists."
   ]
  ]
 },
 {
 ...
 },
 {
  ...
  "body": [
   ...
   "IKE comes in two versions. IKEv1 is the older protocol, using a two-stage phase 1 (main mode with six messages, or the faster but less secure aggressive mode) and then phase 2. IKEv2 is the modern successor: it negotiates in fewer messages, has built-in support for features like NAT traversal and EAP, is more resilient, and is generally preferred for new deployments. Both peers must use the same IKE version to form a tunnel, so a version mismatch prevents negotiation entirely.",
   ...
   "A proposal is a set of algorithms (for example AES-256 with SHA-256) offered during negotiation; the peers pick a combination both support. The Diffie-Hellman group determines the strength of the key exchange (higher-numbered groups are stronger). Perfect forward secrecy (PFS), enabled in phase 2, runs a fresh DH exchange for each new phase 2 key so that compromising one key does not expose past or future keys; without PFS, phase 2 keys derive from phase 1 material.",
   "For ports: IKE negotiation uses UDP 500. When a peer is behind a NAT device, NAT traversal (NAT-T) detects the NAT and moves the encrypted traffic to UDP 4500, encapsulating ESP in UDP so it can cross the NAT. So both UDP 500 and UDP 4500 must be open between peers when NAT is involved.",
 ...
 },
 {
  ...
  "body": [
   ...
   "Second, firewall policies must allow the traffic in both directions. You create a policy from the local LAN interface to the tunnel interface for outbound traffic to the remote site, and a policy from the tunnel interface to the local LAN interface for inbound traffic from the remote site. Both use the appropriate address objects for the local and remote subnets and the required services. Because both directions of an established session must be permitted, missing the return-direction policy is a frequent cause of one-way or failed connectivity. Note that for tunnel traffic you normally do not apply source NAT (the two LANs should see each other's real addresses), unlike Internet policies.",
  ...
  "terms": [
   ...
   [
    ...
    "Firewall policies allowing traffic both from LAN to tunnel and from tunnel to LAN, required for established sessions to pass."
   ],
   [
  ...
  "check": [
   ...
   [
    ...
    "Both directions of an established session must be permitted; a missing return-direction policy causes one-way or failed connectivity."
   ],
   [
