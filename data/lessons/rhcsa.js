...
CertHub.addLessons("rhcsa", [
 ...
 },
 {
  ...
  "body": [
   "Linux configuration lives in plain text files, so editing them quickly and correctly is a core RHCSA skill. RHEL always provides `vi`, and usually `vim` (Vi IMproved) is installed; `nano` is a simpler editor you can install if you prefer it. On the exam you may use either, but vim is guaranteed to be present on minimal systems, so it is worth being comfortable with its basics.",
 ...
 },
 {
  ...
  "body": [
   ...
   "The limits are what the exam asks about. Hard links cannot cross file systems, because inode numbers are only unique within one file system; trying gives an 'Invalid cross-device link' error. Ordinary users cannot create hard links to directories, which prevents loops in the tree. Symbolic links have neither restriction: they can point to directories and to files on other file systems or even to paths that do not exist yet. The price is that they break when the target moves.",
 ...
 },
 {
  ...
  "body": [
   ...
   "Ownership is changed with `chown user:group file` (root only for the user part) and `chgrp group file`. When a process accesses a file, Linux checks only the first matching class: if you are the owner, only the user bits apply, even if the group bits would give you more. New files get their starting permissions from the umask, commonly leaving files at 644 and directories at 755 for normal accounts.",
 ...
 },
 {
  ...
  "body": [
   ...
   "Flatpak repositories are called remotes. A remote is a named source of applications and runtimes, much like a dnf repository, but configured with the `flatpak` command rather than a `.repo` file. Remotes can be added system-wide, available to all users and stored under `/var/lib/flatpak`, which is the default when you run the command as root, or per user with `--user`, stored under `~/.local/share/flatpak`. Make sure the flatpak package itself is installed first (`dnf install flatpak`).",
 ...
 },
 {
  ...
  "body": [
   ...
   "`flatpak install REMOTE APPID` installs an application and, automatically, any runtime it needs. Without a remote name, flatpak searches all configured remotes and asks you to choose. `-y` answers prompts automatically, and `--user` or `--system` selects where it is installed; system installs are the default for root. `flatpak search keyword` finds application IDs across remotes when you do not know the exact name.",
 ...
 },
 {
  ...
  "example": "A script asks for a user name with `read -p 'User: ' u` and runs `id $u`. When someone presses Enter without typing, unquoted $u vanishes and id reports on root. Changing the test to `[ -z \"$u\" ] && exit 1` and using `id \"$u\"` fixes the bug.",
 ...
 },
 {
  ...
  "body": [
   ...
   "Both targets require the root password. If the root account is locked or the password is unknown, you need the initramfs break technique instead, which is covered separately. The names `rescue.target` and `emergency.target` must be spelled exactly; a typo is ignored and the system boots normally."
  ],
  "terms": [
   [
    "GRUB 2",
 ...
 },
 {
  ...
  "body": [
   ...
   "The space in a VG is divided into physical extents (PEs), fixed-size units. Every logical volume is made of a whole number of extents, so the extent size is the granularity of allocation. The default extent size is 4 MiB. `vgcreate -s SIZE` sets a different size at creation, for example `-s 16M` for 16 MiB extents; it must be a power of two. Exam tasks often specify the extent size and then ask for a logical volume measured in extents, so getting `-s` right matters. It is chosen when the VG is created and is not something you would normally change later.",
 ...
 },
 {
  ...
  "body": [
   ...
   "System-wide jobs go in files under `/etc/cron.d/`. These use the same five fields plus a sixth field naming the user to run as, for example `0 * * * * root /usr/local/bin/cleanup.sh`. Forgetting that user field is a common mistake. Scripts dropped into `/etc/cron.daily/` and similar directories are run by anacron, which catches up on jobs missed while the machine was off.",
 ...
 },
 {
  ...
  "body": [
   ...
   "A related tool is `/bin/false`, which simply exits with failure and prints nothing. It also blocks interactive logins, but nologin is preferred on RHEL because it explains why the login failed and can log the attempt. Exam tasks that say the user must not have an interactive shell expect `/sbin/nologin`.",
 ...
 },
 {
  ...
  "example": "Every new user's password must expire after 60 days, and the new hire mia must change her temporary password when she first logs in. You run chage -M 60 mia and chage -d 0 mia; chage -l mia now shows Password must be changed and a maximum of 60.",
 ...
 },
 {
  ...
  "body": [
   "SELinux labels network ports as well as files. Policy says which process types may bind to, meaning listen on, which port types. For example, `httpd_t` may bind to ports labeled `http_port_t`, which by default include 80, 443 and a few others such as 8080. If you configure a service to listen on a port that does not carry its type, SELinux blocks the bind and the service fails to start, even though the configuration is otherwise perfect.",
