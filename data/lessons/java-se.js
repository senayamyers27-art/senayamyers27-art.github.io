...
CertHub.addLessons("java-se", [
 ...
 },
 {
  ...
  "body": [
   ...
   "The compiler uses sealing for exhaustiveness. Because `Payment` has exactly three permitted subtypes, a switch over it with a case for each needs no `default`. Note that `case Voucher v` also covers `GiftVoucher`, since it is a subclass of `Voucher`. `non-sealed` is a contextual keyword with a hyphen, one of the few in Java.",
 ...
 },
 {
  ...
  "body": [
   ...
   "Generics use type erasure: after compilation, type arguments are removed and replaced by their bound (or Object). That is why you cannot write `new T()`, `new T[10]`, `instanceof List<String>` (except with an unbounded wildcard), or overload two methods that differ only by type argument such as `m(List<String>)` and `m(List<Integer>)`; after erasure they have the same signature. Static fields cannot use a class's type parameter either.",
 ...
 },
 {
  ...
  "body": [
   ...
   "A Locale is built from a language code, an optional country (region) code and an optional variant. Language codes are lowercase ISO 639 codes such as `en`, `fr` and `de`. Country codes are uppercase ISO 3166 codes such as `US`, `CA` and `DE`. The string form joins them with an underscore, language first: `en_US`, `fr_CA`. A locale can be language only, like `fr`, but not country only. The standard way to write a locale as a language tag, used on the web, joins them with a hyphen instead: `en-US`.",
