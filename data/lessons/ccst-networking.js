...
CertHub.addLessons("ccst-networking", [
 ...
 },
 {
  ...
  "body": [
   ...
   "In Wireshark you can watch a handshake: filter on `tcp.flags.syn == 1` and you will see SYN and SYN-ACK pairs at the start of each connection. On a computer, `netstat -an` on Windows or `ss -tuna` on Linux lists connections with TCP states such as LISTEN, ESTABLISHED and TIME_WAIT; UDP entries have no connection state (Windows leaves the State column blank and `ss` shows UNCONN) because there is no connection."
  ],
  "terms": [
   [
    "TCP",
 ...
 },
 {
  ...
  "body": [
   "Show commands display information without changing anything, which makes them safe for support technicians to run. Many work in user EXEC mode, but some, such as `show running-config`, need privileged EXEC mode (the # prompt), so it is simplest to run them from there. Knowing which command answers which question is heavily tested and is exactly what an engineer will ask you to run.",
