...
CertHub.addLessons("pcep", [
 ...
 },
 {
  ...
  "body": [
   ...
   "Parameters without defaults are called required parameters, because every call must supply them. The rule is that in the definition all required parameters must come before any parameter that has a default. `def f(a, b=2):` is fine, but `def f(a=1, b):` is a `SyntaxError` (non-default argument follows default argument).",
 ...
 },
 {
  ...
  "body": [
   ...
   "Only one `except` branch runs per exception, even if several could match. To handle different exceptions in the same way, list them as a tuple in one clause: `except (ValueError, TypeError):`. The parentheses are required. To access the exception object, add `as` and a name: `except ZeroDivisionError as e: print(e)` prints the message `division by zero`.",
