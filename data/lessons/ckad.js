...
CertHub.addLessons("ckad", [
 ...
 },
 {
  ...
  "body": [
   "A Pod can hold several containers. They share the same network namespace, so they reach each other on `localhost`, and they can share volumes. That makes it natural to split helper duties into separate containers instead of stuffing them into your application image. The CKAD curriculum names four patterns built on this idea, plus init containers.",
 ...
 },
 {
  ...
  "check": [
   ...
   [
    ...
    "Its command runs to completion instead of staying in the foreground, so the controller keeps restarting it."
   ]
  ]
 },
 {
 ...
 },
 {
  ...
  "body": [
   ...
   "```yaml\ncontainers:\n- name: api\n  image: api:1.0\n  resources:\n    requests:\n      cpu: 250m\n      memory: 128Mi\n    limits:\n      cpu: 500m\n      memory: 256Mi\n```",
   "If you set a limit but no request, Kubernetes sets the request equal to the limit. You can also set resources imperatively: `kubectl set resources deploy/api -c api --requests=cpu=250m,memory=128Mi --limits=cpu=500m,memory=256Mi`, or add `--requests` and `--limits` flags to `kubectl run` output you are generating.",
 ...
 },
 {
  ...
  "body": [
   ...
   "Verify inside the container: `kubectl exec app -- id` should show `uid=1000 gid=3000 groups=2000`, and `kubectl exec app -- touch /test` should fail with 'Read-only file system'. Other container fields you will meet are `privileged` (full host access; avoid), `capabilities` and `seccompProfile`, covered with Pod Security Admission."
  ],
  "terms": [
   [
    "runAsUser",
 ...
 },
 {
  ...
  "example": "A developer applies a default-deny policy in a kind cluster with the default network plugin and is puzzled when requests still succeed. Recreating the cluster with the default network plugin disabled and installing Calico makes the same policy block traffic as expected.",
