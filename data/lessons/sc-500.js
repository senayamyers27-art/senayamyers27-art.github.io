...
CertHub.addLessons("sc-500", [
 ...
 },
 {
  ...
  "body": [
   ...
   "By default all pods in a cluster can talk to each other. Network policies are Kubernetes resources that restrict pod-to-pod traffic by labels, namespaces and ports, implementing micro-segmentation. They require a network policy engine, usually chosen when the cluster is created (some engines can also be enabled on an existing cluster): Azure Network Policy Manager, Calico or Cilium (Azure CNI powered by Cilium).",
 ...
 },
 {
  ...
  "body": [
   ...
   "Audit is available in two tiers. Audit (Standard) is on by default for most organizations and provides searchable audit records for a default retention period (180 days for most record types at the time of writing), a search tool in the Microsoft Purview portal and the `Search-UnifiedAuditLog` cmdlet in Exchange Online PowerShell. Audit (Premium), included with E5-level licensing or add-ons, adds longer retention (one year by default for eligible users, with up to ten years through an add-on), custom audit log retention policies per service or activity, higher bandwidth for API access, and intelligent insight events that are crucial for investigations. Microsoft has since made some of these events, notably MailItemsAccessed and Send, available to Audit (Standard) customers as well, so check which events your licensing logs.",
   "The most important investigation events include MailItemsAccessed, which records when mail data is accessed through mail protocols and clients, helping determine exactly which messages a compromised account or malicious app read; Send, which records messages sent; SearchQueryInitiatedExchange and SearchQueryInitiatedSharePoint, which record what an attacker searched for in mailboxes or SharePoint (a useful signal of intent) and still require Audit (Premium); and other events that help scope a breach. Without these, you may have to assume all mailbox content was exposed.",
