...
CertHub.addLessons("cka", [
 ...
 },
 {
  ...
  "body": [
   ...
   "Check expiry with `kubeadm certs check-expiration`. Renew with `kubeadm certs renew all` (or a single one, such as `kubeadm certs renew apiserver`). After renewing, the control plane static Pods must restart to load the new files; a common approach is to move the manifests out of the directory briefly and back, or restart the kubelet. kubeadm also renews certificates automatically during `kubeadm upgrade apply`, which is one reason clusters upgraded regularly rarely hit expiry.",
   "Kubeconfig files tie it together. A kubeconfig holds clusters (server URL and CA), users (credentials such as a client certificate) and contexts (a cluster plus a user plus an optional namespace). kubeadm writes `admin.conf`, `super-admin.conf` in recent versions, `controller-manager.conf`, `scheduler.conf` and `kubelet.conf` in `/etc/kubernetes`. These embed client certificates, so they are also renewed by `kubeadm certs renew`. Your own `~/.kube/config` is usually a copy of admin.conf, so after renewing you may need to copy it again."
  ],
  "terms": [
   [
    "Static Pod",
 ...
 },
 {
  ...
  "body": [
   ...
   "You can also test from an existing Pod with `kubectl exec -it <pod> -- sh`, if its image has tools, or attach a debug container with `kubectl debug -it <pod> --image=busybox --target=<container>`, which shares the Pod's network namespace. Remember to clean up anything you created without `--rm`."
  ],
  "terms": [
   [
    "kubectl run --rm -it",
 ...
 },
 {
  ...
  "body": [
   ...
   "Some limits to remember: statically created PVs with no StorageClass cannot be expanded through this mechanism, and a volume can only grow as far as its backend allows. hostPath volumes have no real size enforcement at all, so expanding them in a lab changes the numbers but not the disk."
  ],
  "terms": [
   [
 ...
 },
 {
  ...
  "body": [
   ...
   "For all static PVs, the claim must match exactly on class, access mode and size, and capacity is a label for matching rather than a quota: a hostPath PV of 1Gi does not stop a Pod from writing more. Remember that manually created PVs default to the Retain reclaim policy."
  ],
  "terms": [
   [
    "Static provisioning",
