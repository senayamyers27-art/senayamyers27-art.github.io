...
CertHub.addLessons("aws-developer", [
 ...
 },
 {
  ...
  "body": [
   ...
   "Reading the event correctly means knowing its shape for each source. An S3 notification arrives as `event['Records'][i]['s3']['bucket']['name']` and `['object']['key']` (keys are URL-encoded, so decode them). SQS delivers `Records` with `body` as a string you usually parse as JSON. An API Gateway proxy integration event includes `httpMethod` (REST API) or `requestContext.http.method` (HTTP API payload version 2.0), `path` (`rawPath` in 2.0), `headers`, `queryStringParameters`, `pathParameters`, and `body` as a string, possibly Base64-encoded when `isBase64Encoded` is true. Always handle missing fields, because in a REST API event `queryStringParameters` is null when no query string is sent (in a 2.0 event the field is simply absent).",
