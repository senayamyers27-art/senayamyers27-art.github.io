...
CertHub.addLessons("linux-plus", [
 ...
 },
 {
  ...
  "body": [
   ...
   "`tar` (tape archive) bundles files and directories into a single archive while preserving paths, permissions and ownership. The core modes are `-c` create, `-x` extract and `-t` list contents, with `-f` naming the archive file and `-v` for verbose output. Compression is added with a flag: `-z` for gzip (`.tar.gz` or `.tgz`), `-j` for bzip2 (`.tar.bz2`) and `-J` for xz (`.tar.xz`). So `tar -czvf etc-backup.tar.gz /etc` creates a compressed backup, `tar -tzf etc-backup.tar.gz` lists it, and `tar -xzvf etc-backup.tar.gz -C /restore` extracts into a chosen directory. Add `-p` when extracting as root to preserve permissions exactly.",
 ...
 },
 {
  ...
  "body": [
   ...
   "Set these with a leading octal digit or symbolically: `chmod 4755 /usr/local/bin/tool` or `chmod u+s`, `chmod 2770 /srv/team` or `chmod g+s`, `chmod 1777 /shared/drop` or `chmod +t`. A typical team share uses `chmod 2770` plus the sticky bit, giving 3770.",
 ...
 },
 {
  ...
  "body": [
   "Standard permissions are DAC (discretionary access control): the owner decides who gets access. SELinux (Security-Enhanced Linux) and AppArmor add MAC (mandatory access control): a system-wide policy limits what each program may do regardless of file ownership, so a compromised web server cannot read files outside what its policy allows. RHEL-family systems use SELinux; Ubuntu and SUSE use AppArmor.",
